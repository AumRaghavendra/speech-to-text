      ...
      for (let type of mimeTypes) {
        if (MediaRecorder.isTypeSupported(type)) {
          ...
          try {
            ...
            console.log(`Successfully created MediaRecorder with MIME type: ${type}`);
            break;
          } catch (e) {
            console.warn(`Error creating MediaRecorder with MIME type ${type}:`, e);
          }
        }
      }
      
      if (!recorder) {
        console.warn('All MIME types failed, trying without mimeType option');
        try {
          ...
          throw new Error('Cannot create MediaRecorder with any supported options');
        }
      }
